...
function retrieveSavedGame() {
    if (
        sessionStorage.getItem("board") == null ||
        sessionStorage.getItem("players") == null ||
        sessionStorage.getItem("moveNumber") == null
    ) {
...
function loadSavedGame() {
    ...
    for (let n = 0; n < tiles.length; n++) {
        console.log()
        if (tiles[n][1] === 0) {
            //if its a blank tile that had been played, draw a blank tile and then give it the same letter value
...
function onDrop(event) {
    ...
    if (u == null) {
        return
    } //to prevent the error "cannot read parentelement of null"
    origin = u.parentElement.id
    if (origin === "" || destination === "none") {
        return
    }
...
function getRackIds(rackid = "rack") {
    //finds the rack element and grabs the ids of the slots
...
function getTheTile(square) {
    //square is the object (div) that holds the tile
    if (!square) {
        return null
    }
...
function getLettersOnSquare(whichSquare) {
    //check if the square has TW, DL etc
...
function move(fromWhere, toWhere) {
    if (fromWhere === "rack" || toWhere === "rack") {
        return
    }
    if (!checkNameOfLocation(fromWhere) || !checkNameOfLocation(toWhere)) {
        return
    }
    ...
    if (fromWhere[0] === "s" && toWhere[0] === "s") {
        //mmoving tiles around on the rack, making tiles shift  to create space for the incoming tile
    ...
    let blankTile = false
    if (tile.children[2].innerHTML === "0") {
        blankTile = true
    }

    if (tile.children[1].innerHTML === "_" && toBoard) {
        // untouched blank tile at origin
...
function fixSizesAttribs() {
    ...
    for (slot of rackslots) {
        ...
        if (slot.children[0].children[1] == null) {
            break
        }
...
function placeTileOnRack(space_id) {
    ...
    if (!checkNameOfLocation(space_id)) {
        return
    }
    ...
    if (!tile.classList.contains("ghost")) {
        return
    } //Not putting a tile there if one already exists
...
function findEmptyRackPosition(rack = "rack") {
    //Find an empty position on the rack, default is "rack"
    let emptySlot
    let rackSlots = getRackIds(rack)
    for (slot of rackSlots) {
...
function pickTileNumbered(n) {
    //picks the n'th tile from the tile array
    if (tilesArray.length == 0) {
        return
    }
    pickedTile = tilesArray.splice(n, 1)
...
function getSquareNumber(s) {
    //converts a cell address to a numerical value i.e. a1 = 1 , o15 = 225 etc
...
function getHorWords(row) {
    //finds all 2-letter and higher words in row (e.g "a" or "h")
    ...
    for (x of wordbag) {
        //only keep two letter words and above
...
function getVerWords(num) {
    //finds all 2-letter and higher words in column (e.g "a" or "h")
...
function getAllHorWords() {
    ...
    let allHorWords_fin = []
    for (word of allHorWords) {
        //words should have at least one new letter
...
function readLetter(space_id) {
    if (includes("s", space_id) && isEmptyOnRack(space_id)) {
        return
    }
    if (!includes("s", space_id) && isEmptyOnBoard(space_id)) {
        return
    }
...
function isContiguous(arr) {
    //takes an array of letters or numbers and returns true if they are contiguous
...
function checkIfLegal(wordarray) {
    //returns a list of words not in the scrabble dictionary
    let bads = []
...
function getAllIslandWords() {
    //find words which are illegal because they are not adjacent to played letters
...
function displayMove() {
    ...
    let latestscore = `${players[who].name}: <span class=\"redbold\">${readWord(
        playedWord
    )}</span> for <span class=\"redbold\">${score()}</span>`
...
function play() {
    //makes tiles stuck and animates new tiles when play button is pressed
...
function score() {
    //find the scores of all the words in the list
    let tiles = getTilesPlayedNotSubmitted()
    if (tiles.length === 0) {
        return 0
    }
...
function neighbors(sq_id) {
    //get the upper,lower,left and right neighbors of the slot
...
function sendTileBackToRack(space_id) {
    ...
    if (
        !document
            .getElementById(space_id)
            .classList.contains("played-not-submitted")
    ) {
        return
    }
...
let player = {
    ...
    displayData: function () {
        ...
        console.log(
            `I am player ${this.name}, my number is ${this.number} and I have scored ${this.score} points`
        )
    },
    addPoints: function (points) {
        if (isNaN(points)) {
            alert(
                `I am player ${this.name} and I am being asked to add NaN points`
            )
            points = 0
        }
        this.score += points
    ...
    makeRack: function () {
        ...
        if (!document.getElementById(rname)) {
            console.log("rack creation failed")
        }
    },
...
function createAIPlayer(NAME, NUMBER, LEVEL = 2) {
    ...
    AI_player.ai_move = function (from_locs, to_locs) {
        if (typeof from_locs !== typeof to_locs) {
            return
        }
        ...
        if (from_locs.length !== to_locs.length) {
            return
        }
    ...
    AI_player.makeMove = async function () {
        ...
        if (maxPoints - this.score < 25) {
            workerResult = await try_n_tiles(
                this.max_words,
                this.score,
                "worker.js"
            )
            ...
            sortedres = workerResults.sort((a, b) => {
                return b.bestMove["points"] - a.bestMove["points"]
            })
...
function returnToBag(space_id) {
    ...
    if (!includes("s", space_id)) {
        return
    } //only works for tiles on rack
...
function returnAllToBag(slotlist = [1, 2, 3, 4, 5, 6, 7]) {
    //takes an array of numbers and returns those tiles to the bag
    ...
    if (!space_ids.every((element) => r.includes(element))) {
        console.log(
            "rack slots to return from not a subset of slots of this rack"
        )
        return
    }
...
function returnSelected(toExArr) {
    ...
    for (num of toExArr) {
        if (!isNumeric(num) || parseInt(num) < 1 || parseInt(num) > 7) {
            return []
        }
    }
...
function startGame() {
    ...
    if (players[who].isAI) {
        ...
        let titlemsg = ""
        thisIsASavedGame
            ? (titlemsg = "Sorry about the interruption!")
            : (titlemsg = "Let the games begin!")
...
document
    .getElementById("close2lw-list")
    .addEventListener("click", closeMondal2lw)
document
    .getElementById("closevictorybox")
    .addEventListener("click", closevictorybox)
document
    .getElementById("closexchbox")
    .addEventListener("click", closeexchangebox)
document.getElementById("closeall7box").addEventListener("click", closeall7box)
document
    .getElementById("exchSubmit")
    .addEventListener("click", returnExchangeTiles)
document.getElementById("exitgame").addEventListener("click", exitGame)
document
    .getElementById("savethegame")
    .addEventListener("click", () => saveGame(true))
...
for (sqr of sqrs) {
    sqr.addEventListener("dblclick", function () {
        //if there is a tile sitting there, send it back to the rack, otherwise do nothing
